...
# 2. Modern UI CSS (Safe Button-based Cards)
st.markdown("""
    <style>
    ...
    /* Style ONLY the buttons inside the columns to look like the floating white cards */
    ...
    /* Make the text stack inside the button */
...
# Initialize Session State for Active Tab and Chat
if "active_tab" not in st.session_state:
    st.session_state.active_tab = "Model Card"
if "messages" not in st.session_state:
    st.session_state.messages = []

# --- 5. CHAT INTERFACE ---
# Display chat history right below the hero
...
# --- 6. FLOATING CARDS NAVIGATION (Using safe columns + buttons) ---
cols = st.columns(5)
with cols[0]:
    if st.button("📋\nModel Card"): st.session_state.active_tab = "Model Card"
with cols[1]:
    if st.button("🧠\nExplain"): st.session_state.active_tab = "Explain"
with cols[2]:
    if st.button("📉\nScenarios"): st.session_state.active_tab = "Scenarios"
with cols[3]:
    if st.button("🚨\nAnomalies"): st.session_state.active_tab = "Anomalies"
with cols[4]:
    if st.button("📊\nData Intel"): st.session_state.active_tab = "Data Intel"

st.divider()

# --- 7. DYNAMIC CONTENT RENDERING ---

if st.session_state.active_tab == "Model Card":
    ...
    if os.path.exists("submission.csv"):
        ...
        with open("submission.csv", "rb") as file:
            st.download_button(label="Download submission.csv", data=file, file_name="submission.csv", mime="text/csv", use_container_width=True)
...
elif st.session_state.active_tab == "Anomalies":
    ...
    st.divider()
    st.markdown("### 🤖 Historical Reviewer Logs (RAG)")
...
elif st.session_state.active_tab == "Data Intel":
